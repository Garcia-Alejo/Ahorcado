    ...
    while True:
        ...
        if len(entrada) > 1:  
            if entrada == palabras_originales[oponente]:
                palabras_adivinadas[oponente] = palabras_originales[oponente]
                ...
                enviar_resultado_a_jugadores(direccion, oponente)
                break
            else:
                intentos_palabra -= 1
                ...
                if intentos_palabra <= 0:
                    ...
                    enviar_resultado_a_jugadores(oponente, direccion)
                    break
        else:  
...
def enviar_resultado_a_jugadores(ganador, perdedor):
    """
    
    """
    jugadores[ganador].sendall(f"¡Felicidades, ganaste el juego contra {perdedor}!\n".encode())
    jugadores[perdedor].sendall(f"Lo siento, perdiste el juego contra {ganador}.\n".encode())
