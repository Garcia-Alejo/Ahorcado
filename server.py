...
juego_activo_lock = threading.Lock()
jugadores_conectados = threading.Event()
palabras_enviadas = threading.Event()
esperando_turno = {}  # Controla si ya se notificó al jugador que está esperando


def manejar_jugador(conexion, direccion):
    global turno_actual, juego_activo
    ...
    try:
        ...
        historial[direccion] = {"letras_correctas": [], "letras_incorrectas": []}
        uso_revelar[direccion] = False
        esperando_turno[direccion] = False  # Inicializamos el estado de espera

        if len(palabras_originales) == 2:
            palabras_enviadas.set()

        palabras_enviadas.wait()
        conexion.sendall("¡Las palabras están listas! Comienza el juego.\n".encode())
        oponente = [j for j in jugadores if j != direccion][0]

        while True:
            with juego_activo_lock:
                if not juego_activo:
                    break

            with turno_lock:
                if turno_actual == direccion:
                    esperando_turno[direccion] = False  # Reiniciamos el estado de espera
                    conexion.sendall(
                    ...
                    if not esperando_turno[direccion]:
                        conexion.sendall("Es el turno del otro jugador. Espera...\n".encode())
                        esperando_turno[direccion] = True
        ...
        for jugador in jugadores.values():
            if jugador != conexion:
                jugador.sendall("El otro jugador se ha desconectado. El juego ha terminado.\n".encode())
    finally:
        conexion.close()
